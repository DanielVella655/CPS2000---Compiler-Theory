...
since each file directly reflects the tasks as defined in the assignment specification. Then I will discuss the testing strategy
using examples taken from the tests found in the source code.
...
\vspace{-0.5cm}
\begin{center}
    \textit{\footnotesize Note that this invalid ResIR code, pulled from test3\_err.resir, was used to show that the Lexer can only report lexical errors, and 
...
takes as input a list of tokens generated by the \texttt{tokenize} function, and returns a pair of the parsed program, which is a record bundling the extern declarations 
...
\begin{itemize}[nosep]
    ...
    \item \texttt{extern\_functions} - a list of function paths and signatures against calls can be type-checked
\end{itemize}
...
type mismatch when assigning, invalid operator application (such as \texttt{neg} to a variable of \texttt{Bool} type), invalid type cast (these were derived from C99 type casting rules),
...
cleanly map the types that were previously defined in \texttt{parser.ml} onto the corresponding C99 code. The below code illustrates this for types, unary operators, and a RHS example of unary operators:
...
Furthermore, ResIR functions with no parameters are translated into C code with explicit \texttt{void} parameters to be explicit, as seen in tests 3 and 4 below.
...
of the equivalent, compiled C code by translating the validated AST into C that compiles with \texttt{gcc -std=C99 -Wall -Wextra -pedantic generated.c} as required. After finishing task 5 I added the 
...
If an assignment is found with a destination that is not currently live, then that means that this assignment is unused. However this is not the 
...
outside the currently running program. This is checked by the \texttt{safe\_delete} function, and if a statement is deemed to be safe to delete and dead
...
during the DCE pass are not printed in the final C output, avoiding any unused variable flags.
...
with test1.expected.c above has much fewer lines, since most of these were just placed there to test operators.
...
The automated test suite can be run with \texttt{dune test}, and alternatively if one wishes to compile any ResIR code,
...
\begin{table}[H]
    ...
    \begin{tabular}{| c | l |}
        ...
        test2.resir                 & Eliminates many dead assignments, but not store instructions       \\
        test3.resir              & Eliminates many dead assignments, but not call instructions       \\
...
The compiler successfully implements the full pipeline required to translate ResIR into optimized, C99-compliant code. However, due to the scope of the language and the architecture of the passes, there are a few limitations.
